        ...
        const renderMenuItem = React.useCallback(
            ({ label, element: Element, level, className, shortcuts }: TextStyle) => (
                <DropdownMenuItem
                    ...
                    className={cn('flex flex-row items-center group/item justify-between gap-4', {
                        'bg-accent text-primary-foreground': level ? editor.isActive('heading', { level }) : editor.isActive('paragraph')
                    })}
                    aria-label={label}
                >
                    <Element className={className}>{label}</Element>
                    <ShortcutKey
                        className={cn({ 'text-primary-foreground': level ? editor.isActive('heading', { level }) : editor.isActive('paragraph') })}
                        keys={shortcuts}/>
                </DropdownMenuItem>
            ),
